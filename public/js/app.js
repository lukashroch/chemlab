...
$(document).ready(function () {
    ...
    $('form', body).on('submit', function (e) {
        if ($(this).checkSubmit() === true) e.preventDefault();
     ...
     */
    $('#panel-heading-search').on('submit', 'form#form-search', function (e) {
        e.preventDefault();
        $('#data-table').DataTable().draw();
     ...
     */
    $('#action-menu').on('click', 'a.export', function () {
        var button = $(this),
     ...
     */
    $('#chemical-item-move-modal').on('show.bs.modal', function () {
    ...
    $('#chemical-data-menu').on('click', 'a', function (e) {
        ...
        if (type === 'sigma-aldrich' || type === 'all-data') {
            ...
            $.getJSON('/chemical/ajax/parse', { catalog_id: catalogId, callback: 'sigma-aldrich' /*type*/ }).done(function (data) {
                if (data.brand_id === 0) {
                ...
                if (type === 'all-data' && data.brand_id !== 0) getAllCactusData(data.cas, data.name);else $('#chemical-data-icon').removeClass('fa-spin');
            });
    ...
    .on('submit', '#chemical-item-form', function (e) {
        ...
        $.ajax({
            ...
            data: form.serialize(),
            success: function success(data) {
                if (id === '') $('#chemical-items').find('tbody').prepend(data.str);else $('#chemical-items').find('tbody tr.' + id).replaceWith(data.str);
...
function getCactusData(type) {
    ...
    if (type === 'sdf') url += '?operator=remove_hydrogens';

    $('#chemical-data-icon').addClass('fa-spin');
    $.get(url, { string: skipCas ? name : cas, representation: type }).done(function (data) {
        fillCactusData(type, data);
        $('#chemical-data-icon').removeClass('fa-spin');
    }).fail(function () {
        if (skipCas) return;

        $.get(url, { string: name, representation: type }).done(function (data) {
            fillCactusData(type, data);
        }).always(function () {
            $('#chemical-data-icon').removeClass('fa-spin');
        });
...
function fillCactusData(type, data) {
    ...
    if (data.indexOf('<!DOCTYPE') !== -1) return;
...
(function ($) {
    ...
    $.fn.checkSubmit = function () {
        ...
        $('input[type="text"]', $(this)).each(function () {
            var el = $(this);
            if (el.attr('type') === 'text') el.val($.trim(el.val()));
    ...
    $.fn.formatAlert = function (type, text) {
        return $('<div class=\"alert alert-' + type + ' alert-dismissible alert-hidden\"><span class=\"fa fa-common-alert-danger\" aria-hidden=\"true\"></span> ' + text + '<a class=\"close float-right common-alert-close\"><span class=\"fa fa-common-alert-close\" aria-hidden=\"true\" title=\"Close\"></span></a></div>');
    };
    ...
    $.fn.ketcher = function () {
        if (this[0] === 'undefined' || typeof this[0] === 'undefined') return false;

        if ('contentDocument' in this[0]) return this[0].contentWindow.ketcher;else // IE7
