...
import type { Dictionary, HttpRequestConfig } from '@/types';

import { httpService } from '../services';
import { Errors } from './errors';
import { copy, getObjectNestedKeys, merge } from './objects';
...
export default <T = Dictionary>(initData: T, formConfig: FormConfig<T> = {}): Form<T> => {
  ...
  const formDef: FormDef<T> = {
    ...
    async submit<R>(config: HttpRequestConfig): Promise<R> {
      ...
      // Spoof multipart put/patch methods for Laravel
      if (
        this.config.multipart === true &&
        config.method &&
        ['put', 'patch'].includes(config.method)
      ) {
        output.append('_method', config.method);
        config.method = 'post';
      }
