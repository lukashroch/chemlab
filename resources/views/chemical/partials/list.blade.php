<table class="table table-striped table-hover" id="chemical-list">
  ...
  @forelse($chemicals->items() as $chemical)
    <tr class="clickable{{ $chemical->stores ? '' : ' warning' }}"
      ...
      <td>{{ HtmlEx::icon('chemical.show', $chemical->id, $chemical->description ? $chemical->name.' ('.$chemical->description.')' : $chemical->name) }}</td>
      ...
      @if ($action)
        <td class="text-center">
          ...
          {{ HtmlEx::icon('chemical.delete', $chemical->id, $chemical->name) }}
        </td>
      @endif
    </tr>
  @empty
